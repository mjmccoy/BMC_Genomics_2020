...
Figure S4. Non-quantile-normalized expression of gene expression conditional on gene length quantiles.
```{r}
plot_mean_cl <- do.call("rbind", plot.data.list) %>%
  select(
    Gene.ID,
    gene_length,
    variable,
  ...
  ggplot(aes(
      ...
      fill = ifelse(
          ...
          "non-neuronal"),
      group = variable)) + 
  stat_summary(geom = "ribbon", fun.data = mean_cl_normal, alpha = 0.2, aes(col = NULL)) +
  stat_summary(geom = "point", size = 0.1) +
  stat_summary(geom = "line") +
  theme_bw() +
  xlab("Gene length quantiles") +
...
g <- arrangeGrob(grobs = list(plot_mean_cl, plot_loess, plot_gam), ncol = 3)


ggsave(
  filename = "./plots/Figure S4.pdf",
  plot = g,
  device = "pdf", 
  width = 6,
  height = 8,
