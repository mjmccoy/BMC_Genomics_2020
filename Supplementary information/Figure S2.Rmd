...
Figure S2. Intron length versus ordinal position for genes with greater than 2-fold tissue enrichment.
```{r}
g <- do.call("rbind", plot.data.list) %>%
  ...
  mutate(
    fc_cut = factor(
      fc_cut,
      levels = c("fold-difference < 2", "fold-difference > 2")),
    species = factor(
      species,
