...
get_gene_table <- function(species, clade, values = "protein_coding", filters = "biotype", simplified = T, output_dir = "./"){
  ...
  if(!file.exists(filename)){
    ...
    if(simplified == F){
      ...
      exonic_lengths$exonic_length <- exonic_lengths$V2 - exonic_lengths$V1 + 1 # calculate exon lengths
      ...
      agg.genes$gene_length <- agg.genes$end_position - agg.genes$start_position + 1
