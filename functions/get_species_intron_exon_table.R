get_species_intron_exon_table <- function(clade.datasets){
  ...
  for (i in 1:nrow(clade.datasets)) {
    ...
    tryCatch({
      intron_exon_table <- data.frame()
      if(file.exists(paste("./intron_exon_tables/", species, "_intron_exon.RData"))){
        intron_exon_table <- readRDS(paste("./intron_exon_tables/", species, "_intron_exon.RData"))
      } else{
