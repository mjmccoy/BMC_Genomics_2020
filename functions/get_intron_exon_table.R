...
get_intron_exon_table <- function(species, clade, values = "protein_coding", filters = "biotype", output_dir = "./"){
  ...
  if(!file.exists(filename)){
    ...
    data$gene_length <- data$end_position - data$start_position + 1
    data$exon_length <- data$exon_chrom_end - data$exon_chrom_start + 1
      ...
      if(data$strand[i] > 0){
        if(i != nrow(data)){
          if(data$ensembl_transcript_id[i] == data$ensembl_transcript_id[i + 1]){
            data$intron_length[i] <- data$exon_chrom_start[i + 1] - data$exon_chrom_end[i] - 1
          }
        }
      } else {
        if(i != 1){
          if(data$ensembl_transcript_id[i] == data$ensembl_transcript_id[i - 1]){
            data$intron_length[i] <- data$exon_chrom_start[i] - data$exon_chrom_end[i - 1] - 1
          }
        }
      }
    }
